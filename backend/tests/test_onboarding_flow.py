...
async def test_first_time_user_flow(client: AsyncClient, db: AsyncSession):
    ...
    # 2. User logs in (sets httpOnly auth cookies on the client)
    ...
    user_with_profile = await client.get("/api/v1/users/me")
...
@pytest.mark.asyncio
async def test_returning_user_flow(authenticated_client: AsyncClient, test_user: User, db: AsyncSession):
    """Test flow for returning user with existing profile: login -> get profile -> can update."""

    # 1. Create profile for existing user
    initial_profile = await authenticated_client.post(
        "/api/v1/users/profile",
        json={
            "age": 30,
            "gender": "male",
    ...
    # 2. "User logs out and logs back in" -> fresh cookies for the same user
    ...
    user_response = await authenticated_client.get("/api/v1/users/me")
    assert user_response.status_code == 200
    user_data = user_response.json()
...
async def test_profile_partial_updates(
    ...
    create_response = await authenticated_client.post("/api/v1/users/profile", json=initial)
